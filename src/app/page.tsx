    ...
    <div className="bg-gray-50 text-slate-900 antialiased selection:bg-orange-500 selection:text-white" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <nav className="fixed w-full z-50 bg-white/85 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            {/* Mobile Menu Button */}
            <button
              className="md:hidden text-slate-600 hover:text-orange-600 transition-colors p-2 rounded-lg hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
      ...
      <div className="relative pt-20 min-h-[85vh] flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-4 text-center mt-10">
          ...
          {/* Search Bar */}
          <form action="/properties" method="GET" className="bg-white p-2 rounded-2xl shadow-2xl max-w-4xl mx-auto flex flex-col md:flex-row gap-2 items-center transform hover:scale-[1.01] transition-all duration-300 focus-within:ring-4 focus-within:ring-orange-500/20">
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 transition-colors focus-within:bg-white focus-within:border-orange-200">
              <Label htmlFor="hero-location" className="sr-only">Location (e.g. Lahore)</Label>
              <MapPin className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input id="hero-location" name="city" type="text" placeholder="Location (e.g. Lahore)" className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" />
            </div>
            <div className="hidden md:block w-px h-8 bg-gray-200"></div>
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 transition-colors focus-within:bg-white focus-within:border-orange-200">
              <Label htmlFor="hero-query" className="sr-only">Search Venues or Services...</Label>
              <Search className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input id="hero-query" name="q" type="text" placeholder="Search Venues or Services..." className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" />
            </div>

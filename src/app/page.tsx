    ...
    <div className="bg-gray-50 text-slate-900 antialiased selection:bg-orange-500 selection:text-white" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden md:flex items-center space-x-8">
              <Link href="/properties" className="text-slate-600 hover:text-orange-600 font-medium transition-colors">Venues</Link>
              <Link href="/services" className="text-slate-600 hover:text-orange-600 font-medium transition-colors">Services</Link>
            ...
            {/* Mobile Menu Button */}
            <button className="md:hidden text-slate-600 p-2 hover:bg-black/5 rounded-lg transition-colors cursor-pointer" aria-label="Open mobile menu">
      ...
      <div className="relative pt-20 min-h-[85vh] flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-4 text-center mt-10">
          ...
          {/* Search Bar */}
          <form action="/properties" method="GET" className="bg-white p-2 rounded-2xl shadow-2xl max-w-4xl mx-auto flex flex-col md:flex-row gap-2 items-center transform hover:scale-[1.01] transition-transform duration-300 focus-within:ring-2 focus-within:ring-primary">
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 transition-colors focus-within:border-primary/30">
              <MapPin className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input name="city" type="text" placeholder="Location (e.g. Lahore)" className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" aria-label="Location" />
            </div>
            <div className="hidden md:block w-px h-8 bg-gray-200"></div>
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 transition-colors focus-within:border-primary/30">
              <Search className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input name="q" type="text" placeholder="Search Venues or Services..." className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" aria-label="Search" />
            </div>
            <button type="submit" className="w-full md:w-auto px-10 h-14 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold rounded-xl transition-all shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2 cursor-pointer">
              Search
            </button>
          </form>
        </div>

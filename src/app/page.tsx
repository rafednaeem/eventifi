    ...
    <div className="bg-gray-50 text-slate-900 antialiased selection:bg-orange-500 selection:text-white" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="hidden md:flex items-center space-x-8">
              ...
              <button className="px-6 py-2.5 rounded-full bg-slate-900 text-white font-medium hover:bg-orange-600 active:scale-95 transition-all duration-300 shadow-lg hover:shadow-orange-500/30 transform hover:-translate-y-0.5">
                Get Started
              </button>
            </div>
            {/* Mobile Menu Button */}
            <button type="button" aria-label="Toggle menu" className="md:hidden text-slate-600 p-2 hover:bg-slate-100 active:scale-95 rounded-lg transition-all">
      ...
      <div className="relative pt-20 min-h-[85vh] flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 w-full max-w-5xl mx-auto px-4 text-center mt-10">
          ...
          {/* Search Bar */}
          <form action="/search" method="GET" className="bg-white p-2 rounded-2xl shadow-2xl max-w-4xl mx-auto flex flex-col md:flex-row gap-2 items-center transform hover:scale-[1.01] focus-within:scale-[1.01] focus-within:ring-2 focus-within:ring-orange-500/20 transition-all duration-300">
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 focus-within:border-orange-400 focus-within:bg-white transition-colors">
              <MapPin className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input type="text" name="city" placeholder="Location (e.g. Lahore)" aria-label="Location" className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" />
            </div>
            <div className="hidden md:block w-px h-8 bg-gray-200"></div>
            <div className="flex-1 w-full flex items-center px-4 h-14 bg-gray-50 rounded-xl border border-transparent hover:border-orange-200 focus-within:border-orange-400 focus-within:bg-white transition-colors">
              <Search className="text-orange-500 w-5 h-5 mr-3 shrink-0" />
              <input type="text" name="q" placeholder="Search Venues or Services..." aria-label="Search venues or services" className="bg-transparent w-full outline-none text-slate-700 placeholder-slate-400 font-medium" />
            </div>
            <button type="submit" className="w-full md:w-auto px-10 h-14 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 active:scale-95 text-white font-bold rounded-xl transition-all shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2">
              Search
            </button>
          </form>
        </div>
      ...
      <footer className="bg-slate-950 text-slate-400 py-16 border-t border-slate-900">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <Link href="/" className="inline-flex items-center justify-center gap-2 mb-4 text-white font-bold text-2xl hover:opacity-80 active:scale-95 transition-all">
